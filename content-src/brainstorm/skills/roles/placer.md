...
- `unmatched` — the pool members that matched no taxonomy node. Each carries its `term`, its
  ...
  spellings), and its `origins` (who stated it, on which paper).
